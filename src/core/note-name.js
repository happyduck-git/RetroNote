...
// 파일명 = note_YYYY-MM-DD_HH-MM-SS.txt. (구버전 분 단위 note_..._HH-MM.txt 도 유효로 인정.)
import { pad2 } from "./dom.js";

// 보안 경계: 아래 anchored allowlist 가 fs 진입을 막는 본질이다.
// ASCII 숫자/하이픈/리터럴 .txt 만 허용 → 절대경로·NUL·유니코드 homoglyph·
// Windows 예약어(CON/AUX)·후행 점/공백을 전부 자동 거부. 절대 약화하지 말 것.
const NOTE_FILENAME_RE = /^note_\d{4}-\d{2}-\d{2}_\d{2}-\d{2}(-\d{2})?\.txt$/;
...
export function noteFilename(date) {
  return `note_${formatTimestamp(date)}.txt`;
}
