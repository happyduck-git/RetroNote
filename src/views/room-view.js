...
import { getRoomNickname, getClientId, openRoom, closeRoom, saveRoom, changeRoomNickname, HISTORY_PAGE_SIZE } from "../chat/session.js";
...
function createScrollAnchor(list) {
  ...
  function captureAnchor() {
    ...
    for (const row of list.children) {
      // 날짜 구분선(dataset.id="date-…")은 앵커로 잡지 않는다. 같은 날짜 과거를 prepend 하면
      // withDateDividers 가 구분선을 더 위로 옮겨, 구분선에 건 앵커는 화면을 아래로 점프시킨다.
      // 메시지 행에 걸면 그 메시지는 제자리에 남고 추가분은 화면 밖 위로 쌓인다.
...
export const roomView = {
  ...
  async mount(screenEl, params, ctx) {
    ...
    // replaceChildren 에 지워진다. loadingOlder 동안만 보이고, 바닥에 닿으면 "처음입니다" 표시.
