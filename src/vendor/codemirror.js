// CodeMirror 6 (@codemirror/* 6.x, @lezer/markdown) — vendored browser bundle (esbuild, format=esm, platform=browser, target=es2020).
// Do not edit by hand. Regenerate only when bumping CM:
//   npx esbuild vendor-src/codemirror.entry.js --bundle --format=esm --platform=browser --target=es2020 --legal-comments=none --outfile=src/vendor/codemirror.js
// Exports: EditorView, Decoration, WidgetType, ViewPlugin, keymap, placeholder, history, historyKeymap, defaultKeymap, syntaxTree, markdown, markdownLanguage, GFM.
...
var identifierTokens = (id2, varName, callee2) => (input, stack) => {
  for (let inside = false, dashes = 0, i2 = 0; ; i2++) {
    ...
    if (isAlpha(next) || next == dash2 || next == underscore || inside && isDigit(next)) {
      if (!inside && (next != dash2 || i2 > 0)) inside = true;
